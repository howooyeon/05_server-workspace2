...
public class MemberEnrollController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 기본생성자로 생성한 후 setter 메소드 이용해서 담기 => 담으려고 하는게 소량일 때
		...
		Member m = new Member(userId, userPwd, userName, phone, email, address, interest);
			...
			// 4) 처리결과를 가지고 사용자가 보게될 응답뷰 지정 후 포워딩 또는 url 재요청
			if(result > 0) { // 성공
				
				HttpSession session = request.getSession();
				session.setAttribute("alertMsg", "성공적으로 회원가입이 되었습니다."); // request는 포워딩 방식일 때만 가
