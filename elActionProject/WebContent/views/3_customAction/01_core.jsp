...
<body>
	...
	* 변수 선언과 동시에 초기화 (c:set var="" value="" [scope=""])
	...
	- 해당 변수를 어떤 scope에 담아둘껀지 지정가능함 (생략시 기본적으로는 pageScope에 담김)
	=> 즉, 해당 scope에 setAttribute를 통해서 key-value 형태로 데이터를 담아놓는 거라고 생각하면 됨
	=> c:set으로 선언된 변수는 EL로 접근해서 사용가능(스크립팅 원소로는 접근 불가!)
	...
	<c:set var="num2" value="20" scope="request"/> <!-- request.setAttribute("num1", "20") -->
	...
	* 변수(데이터) 출력 (c:out value="출력하고자하는값" [default="기본값" [ escapeXml ="true | false"]])
	...
	- 각 조건들을 c:choose의 하위 요소로c:when을 통해서 작성 (else문 역할 == c:otherwise))
	</pre>
	...
	- 향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근하고자하는 객체 (배열|컬랙션)" [varStatus="현재 접근된 요소의 상태값을 보관할 변수명?"]
	...
	<ol>
		<c:forTokens items="${ device }" delims=",./">
			<li>${ d }</li>
		</c:forTokens>
	</ol>
	...
	<c:url var="" value="list.do">
		<c:param name="cpage" value="1"></c:param>
